		...
		<div class="row ipad-width2">
            ...
            <div class="movie-item-style-2">
                <div class="mv-item-infor">
                    <h6 ><a id="myCheck" >Movies in <?php echo $location; ?></a></h6>
                </div>
            </div>
            <?php 
            $query = $this->db->query("SELECT DISTINCT movie_id FROM sessions where theater_id=$id ;");
			foreach ($query->result_array() as $session)
			{ 
                $a = $session['movie_id'];
                $movie_query = $this->db->query("SELECT  * FROM movie where id=$a ;"); 
                foreach($movie_query->result_array() as $movie){
                ?>
				<div class="movie-item-style-2">
